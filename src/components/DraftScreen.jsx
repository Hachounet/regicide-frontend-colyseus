...
const DraftScreen = () => {
  ...
  const renderPlayer = (player, visualIndex) => {
    ...
    return (
      ...
      >
        ...
        <motion.div
          className={`w-20 h-20 rounded-full flex items-center justify-center font-bold text-2xl shadow-xl border-2 transition-all duration-300
        ...
        >
          ...
          <span className="absolute inset-0 rounded-full pointer-events-none" style={{
            boxShadow: isMe
              ? '0 0 32px 8px rgba(59,130,246,0.25)' // bleu
              : '0 0 32px 8px rgba(156,163,175,0.18)' // gris
          }} />
          <span className="z-10 text-white drop-shadow-lg">
            {player.pseudo?.charAt(0)?.toUpperCase() || '?'}
          </span>
        </motion.div>
        {/* Nom du joueur modernisé */}
  ...
  if (draftPhase === 'revealing_king' && myPlayer?.secretKing) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        >
          <h2 className="text-3xl font-bold mb-6 text-white">Votre Roi</h2>
          <motion.div
  ...
  return (
    <div className="relative w-full h-screen  overflow-hidden">
      ...
      {/* Joueurs positionnés en haut à droite */}
      ...
      {draftPhase === 'drafting' && myPlayer?.draftPack?.length > 0 && (
        <motion.div
          ...
          style={{ marginTop: '-50px', marginLeft: "-27%" }} // Parfaitement centré verticalement
        >
      ...
      {/* Main du joueur en bas - Main complète comme GameScreen */}
      ...
      {draftPhase === 'waiting' && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
          <div className="text-white text-xl mb-4">En attente du début du draft...</div>
